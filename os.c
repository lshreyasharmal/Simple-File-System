...
int createFiles( char* filename, int nbytes)
{
	...
	for(i=0;i<=100;i++)
	{
		if(filesystem[i].location== -1)
		{
			...
			fprintf(f,"no_inodes=%d no_datablocks=%d beg_datablocks=%d beg_inodes=%d fileSystemID=%d locationFS=%d diskNum=%d",160,56,9,4,i,filesystem[i].location,superblock[i].fss.diskinuse.diskNum);
			fprintf(f," diskname=");
			fprintf(f,superblock[i].fss.diskinuse.diskName);
...
void print_FileList(int fileSystemId)
{
	// for (int i=0;i< 100;i++)
	// {
	// 	if(Files[i].fs.fileSystemID==fileSystemId)
	// 			{
	// 				printf("%s\n",Files[i].filename);
	// 			}
	// 			else
	...
	for(i=0;i<33;i++)
{
	...
	printf("\n");
		...
		{
			FILE *file;
				// get the disk a)nd the location in the disk
		int i,j;
		for(i=0;i<100;i++)
		{
			if(filesystem[i].fileSystemID==fileSystemId)
...
int writeFile( int Disk, int blockNum, void* block, char* filename)
{
	int i,j,bitmap;
	for(i=0;i<100;i++)
	{
				if(Inode[i].inodeNum==-1)
				{
					...
					//  sprintf(s4,"%d",Inode[i].fs.diskinuse.diskName);
					 ...
					 Inode[i].datablock = blockNum; //yeh batata hai ki iss inode ki file ka data kahan hai
...
int i,j,k; char str[100]; char s1[100];
for(i=0;i<33;i++)
{
	//printf("%d\n",i );
	strcpy(buffer,"HELLO");
	...
	sprintf(s1,"%d",i);strcat(str,s1);
	//printf("%s",str);
	for(k=0;k<strlen(str);k++)
		arr[i][k]=str[k];
	int 	x= writeFile(0,i+9,block,str);
	//strcpy(Files[i].filename,str);
}
	//createFiles("disk.txt",64);
